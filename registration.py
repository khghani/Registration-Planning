...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs(vertex_idx):
            if self.vertices[vertex_idx].depth != -1:
                return self.vertices[vertex_idx].depth
            maximum_depth = 0
            for adjacent in self.get_adjacent_vertices(vertex_idx):
                adjacent_depth = dfs(adjacent)
                maximum_depth = max(maximum_depth, adjacent_depth + 1)
            self.vertices[vertex_idx].depth = maximum_depth
    ...
    def get_registration_plan(self):
        ...
        """
        vertex_length = len(self.vertices)
        audit = [0] * vertex_length
        for ult in range(vertex_length):
            for vertex in self.get_adjacent_vertices(ult):
                audit[vertex] += 1
        semesters = []
        while True:
            ongoing_sem = []
            for idx in range(vertex_length):
                if audit[idx] == 0 and not self.vertices[idx].visited:
                    ongoing_sem.append(idx)
            if not ongoing_sem:
                break
            ongoing_sem.sort(
                key = lambda idx: (-self.vertices[idx].depth, self.vertices[idx].label))
            courses = ongoing_sem[:4]
            sem_tags = [self.vertices[idx].label for idx in courses]
            semesters.append(sem_tags)
            for idx in courses:
                self.vertices[idx].visited = True
                for adjacent in self.get_adjacent_vertices(idx):
                    audit[adjacent] -= 1
        for vertex in self.vertices:
...
def main():
    ...
    """
    graph = Graph()
    vertices_value = int(sys.stdin.readline().strip())
    for _ in range(vertices_value):
        tags = sys.stdin.readline().strip()
        graph.add_vertex(tags)
    edges_value = int(sys.stdin.readline().strip())
    for _ in range(edges_value):
        start_tag, end_tag = sys.stdin.readline().strip().split()
        start = graph.get_index(start_tag)
        end = graph.get_index(end_tag)
